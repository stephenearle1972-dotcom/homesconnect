...
const PRIVATE_ID = process.env.HOMESCONNECT_PRIVATE_SHEET_ID || SHEET_ID; // Offers/MaoListings live here
...
async function append(sheetId, tab, row) { const t = await gtok(); await sapi(t, 'POST', `/${sheetId}/values/${encodeURIComponent(tab + '!A1')}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`, { values: [row] }); }
async function delByPrefix(sheetId, tab, prefix) { const t = await gtok(); const meta = await sapi(t, 'GET', `/${sheetId}?fields=sheets.properties(title,sheetId)`); const sid = meta.sheets.find(s => s.properties.title === tab).properties.sheetId; const v = (await sapi(t, 'GET', `/${sheetId}/values/${encodeURIComponent(tab + '!A1:BZ2000')}`)).values || []; for (let i = v.length - 1; i >= 1; i--) { if ((v[i][0] || '').startsWith(prefix)) await sapi(t, 'POST', `/${sheetId}:batchUpdate`, { requests: [{ deleteDimension: { range: { sheetId: sid, dimension: 'ROWS', startIndex: i, endIndex: i + 1 } } }] }); } }
...
await append(PRIVATE_ID, 'Offers', offerRow({ id: 'OFDEMOA', listing_id: LISTING_ID, buyer_name: 'Lerato Demo', buyer_phone_verified: '27825552001', buyer_email: `${INBOX}+demobuyerA@gmail.com`, buyer_entity_type: 'individual', proposed_price: '3350000', funding_method: 'bond', bond_amount: '3350000', cash_contribution: '', deposit_amount: '150000', subject_to_sale: 'no', subject_to_sale_note: '', occupation_date: '2026-08-01', proposal_expiry: '2026-06-20', note_to_seller: 'We love the garden and can move quickly.', note_for_conveyancer: '', ack_nonbinding: 'yes', consent_share_seller: 'yes', consent_share_conveyancer: '', status: 'submitted', selected_conveyancer: '', seller_action_log: auditA, created_at: iso, updated_at: iso }));
await append(PRIVATE_ID, 'Offers', offerRow({ id: 'OFDEMOB', listing_id: LISTING_ID, buyer_name: 'Pieter Demo', buyer_phone_verified: '27825552002', buyer_email: `${INBOX}+demobuyerB@gmail.com`, buyer_entity_type: 'company', proposed_price: '3450000', funding_method: 'mixed', bond_amount: '3000000', cash_contribution: '450000', deposit_amount: '200000', subject_to_sale: 'yes', subject_to_sale_note: 'Selling our Centurion townhouse first.', occupation_date: '2026-09-01', proposal_expiry: '2026-06-25', note_to_seller: 'Flexible on dates.', note_for_conveyancer: 'Company purchase — VAT vendor.', ack_nonbinding: 'yes', consent_share_seller: 'yes', consent_share_conveyancer: '', status: 'submitted', selected_conveyancer: '', seller_action_log: JSON.stringify([{ t: iso, actor: 'buyer', event: 'submitted', consents: { ack_nonbinding: true, share_seller: true, ack_conveyancer_later: true, privacy: true, marketing: false } }]), created_at: iso, updated_at: iso }));
